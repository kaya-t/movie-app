...
namespace MovieApp.Api.Providers
{
    public class CinemaworldClient : IMovieProviderClient
    {
        ...
        public CinemaworldClient(HttpClient http, IOptions<ProviderOptions> opts)
        {
            _http = http;
            _opts = opts.Value;
            ...
            if (string.IsNullOrWhiteSpace(token))
                throw new ProviderException("cinemaworld", "API token missing");
        ...
        {
            var resp = await _http.GetAsync($"{_opts.Cinemaworld.BaseUrl}/movies", ct);
        ...
        public async Task<MovieDetail?> GetMovieAsync(string id, CancellationToken ct)
        {
            try
            {
                var resp = await _http.GetAsync($"{_opts.Cinemaworld.BaseUrl}/movie/{id}", ct);
